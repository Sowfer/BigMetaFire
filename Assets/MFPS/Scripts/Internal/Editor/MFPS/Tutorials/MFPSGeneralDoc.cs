...
public class MFPSGeneralDoc : TutorialWizard
{
    ...
    new Steps { Name = "Game Input", StepsLenght = 0, DrawFunctionName = nameof(GameInputDoc) },    
    ...
    void DrawGameTexts()
    {
        DrawText("If you want to change some text in the game or maybe just change the text grammar, the major part of the text is directly set in the UI Text's components which are located inside the canvas objects in each scene of the game, but also there is some text that is set in runtime by code, to make easy for you to find all these texts we have put all those in just 1 script: <b>bl_GameText.cs</b>\n \nin this script you will find all the text that established by code in runtime, you can modify them as you want from that script, this facilitates the work of for example adding your own location system.");
        DrawServerImage("img-22.png");
    }

    void GameInputDoc()
    {
        DrawText("A critical aspect of shooter games <b><size=8>(or all games in general)</size></b> is user control, but this is kinda special in action games since players usually have their own way to set up the inputs with which they feel comfortable playing, MFPS by default have the \"standard\" input set up which is set in the most fps games.\n \nBut you probably want to modify the default set up or you may want to integrate a third-party Input Management system like <b>InControl</b> or <b>Rewired</b> which supports multiple input control devices, MFPS doesn't support third-party assets by default but make them easier to integrate.\n \nFor it, MFPS handles all the inputs from one script: <color=#00CEFFFF>bl_GameInput.cs</color>, in that script you will find all the inputs that are used in the game, you can go from simply modify the default Input key to use your own Input Manager methods instead of the default Unity Input.GetKey methods.\n \nYou can identify which input is for what game action by the function name <i>(Fire, Reload, Jump, etc...)</i>");
        DownArrow();
        DrawHyperlinkText("Optional to that, MFPS has an available addon called <link=https://www.lovattostudio.com/en/shop/addons/input-manager-addons/>Input Manager</link> which allows changing the input binding in-game for keyboard and console gamepads.");
        DownArrow();
        DrawSpoilerBox("What about the new Unity Input System?", "MFPS doesn't support the new Input System by default yet but is planned to be added in a future update.");
    }
